...
namespace ftx
{
    ...
    json RESTClient::list_futures(const std::string future)
    {
        auto response = http_client.get("futures/" + future);
        return json::parse(response.body());
    }
    json RESTClient::get_future_stats(const std::string future)
    {
        auto response = http_client.get("futures/" + future + "/stats");
        return json::parse(response.body());
    }
